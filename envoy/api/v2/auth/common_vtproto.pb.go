...
func (m *PrivateKeyProvider_Config) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *PrivateKeyProvider_Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	i := len(dAtA)
	if m.Config != nil {
...
func (m *CertificateValidationContext) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.RequireOcspStaple != nil {
		size, err := (*wrapperspb.BoolValue)(m.RequireOcspStaple).MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
